...
public class ProductUpdateRequestTest {
    private static final Logger logger = Logger.getLogger(ProductUpdateRequestTest.class);
    ...
    public void validateInputRequest() throws Exception {
        ...
        try {
            for (File file : files) {
                logger.debug("product update request case => " + file.getName());
                ProductUpdateRequest productUpdateRequest = mapper.readValue(file, ProductUpdateRequest.class);
                List<String> actual = new ArrayList<>();

                Set<ConstraintViolation<ProductUpdateRequest>> errors = validator.validate(productUpdateRequest);
                for (ConstraintViolation<?> error : errors) {
                    actual.add(error.getPropertyPath().toString() + ": " + error.getMessage());
                }

                List<String> expected = mapper.readValue(FileUtils.readFileToString(new File(FilenameUtils.concat("src/test/resources/case-input-request/productUpdateRequest/expected", file.getName())), StandardCharsets.UTF_8), new TypeReference<List<String>>() {});
    ...
    private File[] readTestCase() throws Exception {
        File folder = new File("src/test/resources/case-input-request/productUpdateRequest/request");
