...
public class ProductRequestTest {
    private static final Logger logger = Logger.getLogger(ProductRequestTest.class);
    ...
    public void validateInputRequest() throws Exception {
        ...
        try {
            for (File file : files) {
                logger.debug("product request case => " + file.getName());
                ProductRequest productRequest = mapper.readValue(file, ProductRequest.class);
                List<String> actual = new ArrayList<>();

                Set<ConstraintViolation<ProductRequest>> errors = validator.validate(productRequest);
                for (ConstraintViolation<?> error : errors) {
                    actual.add(error.getPropertyPath().toString() + ": " + error.getMessage());
                }

                List<String> expected = mapper.readValue(FileUtils.readFileToString(new File(FilenameUtils.concat("src/test/resources/case-input-request/productRequest/expected", file.getName())), StandardCharsets.UTF_8), new TypeReference<List<String>>() {});
    ...
    private File[] readTestCase() throws Exception {
        File folder = new File("src/test/resources/case-input-request/productRequest/request");
